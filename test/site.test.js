...
test('landing page has truthful SEO metadata and canonical URL', () => {
  ...
  assert.ok(title.length >= 50 && title.length <= 60, `title should be 50â€“60 characters (got ${title.length})`);
  assert.ok(description, 'description is required');
  assert.ok(description.length >= 150 && description.length <= 160, `description should be 150â€“160 characters (got ${description.length})`);
